...
func Test(env goext.IEnvironment) {
	...
	Describe("Example tests", func() {
		...
		It("Smoke test CRUD", func() {
			Expect(schema.Create(entry, nil)).To(Succeed())
			Expect(schema.Update(entry, nil)).To(Succeed())
			Expect(schema.Delete(goext.Filter{"id": entry.ID}, nil)).To(Succeed())
		})

		FIt("Should change name in pre_update event", func() {
			Expect(schema.Create(entry, nil)).To(Succeed())
			entry.Name = "random name"
			Expect(schema.Update(entry, nil)).To(Succeed())
			Expect(entry.Description).To(Equal("name changed in pre_update event"))
		})
	})
}
