...
func Test(env goext.IEnvironment) {
	...
	Describe("Example tests", func() {
		...
		It("Smoke test CRUD", func() {
			Expect(schema.Create(entry, ctx)).To(Succeed())
			Expect(schema.Update(entry, ctx)).To(Succeed())
			Expect(schema.Delete(goext.Filter{"id": entry.ID}, ctx)).To(Succeed())
		})
